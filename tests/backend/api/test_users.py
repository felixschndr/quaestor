...
def test_list_users_returns_only_the_current_user(http_client: TestClient):
    ...
    assert [user["name"] for user in response.json()] == ["bob"]


def test_get_own_user_includes_credentials_and_balance(http_client: TestClient):
    user_id = register(http_client, name="alice").json()["id"]
...
def test_update_user_changes_name(http_client: TestClient):
    user_id = register(http_client, name="alice").json()["id"]

    response = http_client.patch(f"/users/{user_id}", json={"name": "renamed"})
