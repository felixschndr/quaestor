...
describe('OverviewView hero extras', () => {
  ...
  it('stays silent about syncing while every bank is current', () => {
    ...
    render_(
      buildUser({
        balance: AMOUNT_S,
        credentials: [{ ...buildCredential(), last_successful_sync_timestamp: fresh }],
      }),
    )
    const syncButton = screen.getByRole('button', { name: 'Sync all accounts' })
  ...
  it('leaves banks with syncing switched off out of the staleness check', () => {
    ...
    render_(
      buildUser({
        balance: AMOUNT_S,
        credentials: [
          { ...buildCredential(), id: 1, last_successful_sync_timestamp: fresh },
          {
            ...buildCredential(),
            id: 2,
            accounts: [{ ...buildCredential().accounts[0], id: 9 }],
            last_successful_sync_timestamp: long_ago,
            sync_enabled: false,
          },
        ],
      }),
