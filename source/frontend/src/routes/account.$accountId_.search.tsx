...
function TransactionSearchPage() {
  ...
  return (
    <TransactionSearchView
      ...
      onSubmit={(filters) =>
        navigate({
          search: { ...filters, submitted: '1' } as TransactionSearchParams,
          replace: false,
        })
      }
    />
...
export function TransactionSearchView({ accountId, search, onSubmit }: TransactionSearchViewProps) {
  ...
  return (
    <main className="mx-auto flex min-h-full max-w-2xl flex-col gap-6 p-4">
      ...
      {search.submitted ? (
        <SearchResults accountId={accountId} filters={toFilters(search)} />
      ) : null}
    </main>
  )
}
...
function SearchForm({
  ...
  const [draft, setDraft] = useState<TransactionFilters>(toFilters(initial))
...
function SearchResults({ accountId, filters }: { accountId: number; filters: TransactionFilters }) {
  const { t } = useTranslation()
  const query = useSearchTransactions(accountId, filters)
    ...
    <section aria-label={t('search.resultsHeading')} className="flex flex-col gap-2">
      ...
      <ul className="flex flex-col">
        {results.map((transaction) => (
          <ResultRow key={transaction.id} accountId={accountId} transaction={transaction} />
        ))}
      </ul>
    </section>
  )
...
/**
 * Strip the meta-only `submitted` flag and re-emit only the actual filter
...
function toFilters(search: TransactionSearchParams): TransactionFilters {
  const { submitted: _submitted, ...filters } = search
  void _submitted
  return filters
}
