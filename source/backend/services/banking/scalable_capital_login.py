...
def _cleanup_expired_pending_logins() -> None:
    ...
    if expired:
        logger.debug(f"Cleaned up {len(expired)} expired pending login(s)")
...
def _next_line(sink: "queue.Queue[str | None]", deadline: datetime, credential_id: int) -> str | None:
    ...
    if remaining <= 0:
        raise InvalidTwoFactorError(f"Login timed out for credential {credential_id}")
    try:
        return sink.get(timeout=remaining)
    except queue.Empty as e:
        raise InvalidTwoFactorError(f"Login timed out for credential {credential_id}") from e
    ...
    while True:
        ...
        if line is None:
            raise InvalidTwoFactorError(
                f"Login process ended without an authorization URL for credential {credential_id}"
            )
        ...
        if authorization_url is None:
            if stripped.lower().startswith(_AUTHORIZATION_URL_PREFIX):
                ...
                if not url_line or not url_line.strip():
                    raise InvalidTwoFactorError(
                        f"Login did not print an authorization URL for credential {credential_id}"
                    )
...
def start(credential_id: int) -> tuple[str, str, str | None, datetime]:
    logger.info(f"Initiating device-code login for credential {credential_id}")
    ...
    logger.info(f"Device-code challenge issued for credential {credential_id}")
...
def write_session_state(config_dir: Path, session_state: dict[str, str]) -> None:
    ...
    if not encoded:
        raise ValueError("Session state does not contain a CLI config archive")
...
def complete(challenge_token: str, credential_id: int) -> dict[str, str]:
    ...
    if pending_login is None or pending_login.credential_id != credential_id:
        error_message = f"Unknown or expired login challenge for credential {credential_id}. " "Start the sync again."
    ...
    try:
        ...
        error_message = f"Login for credential {credential_id} has not finished yet in the browser; try again."
        logger.warning(error_message)
    ...
    if pending_login.process.returncode != 0:
        error_message = f"Login failed for credential {credential_id}"
        logger.warning(error_message)
    ...
    _remove_config_dir(pending_login.config_dir)
    logger.info(f"Login completed for credential {credential_id}")
